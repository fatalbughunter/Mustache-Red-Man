<template>
  <div class="crash-controls">
    <img
      src="@/assets/img/christmas/whiteSnow.png"
      alt="Snow"
      class="crash-controls-snow"
    />
        ...
        <div class="bets-header">
          ...
          <div class="header-amount">
            ...
            <div class="amount-value">
              <span>{{
                crashFormatValue(crashGetBetsAmount).split(".")[0]
              }}</span
...
export default {
  ...
  methods: {
    ...
    handleAutoCashout() {
      ...
      if (
        this.crashMode === "manual" &&
        this.crashGame &&
        this.crashGame.state === "rolling"
      ) {
    ...
    crashBetButton() {
      ...
      const amount = parseFloat(Number(this.crashAmount)) || 0;
      const autoCashout =
        this.crashAutoCashout === null || this.crashAutoCashout.trim() === ""
          ? 0
          : Math.round(parseFloat(this.crashAutoCashout) * 100);
    ...
    crashBetCashout() {
      ...
      if (this.socketCrash === null || this.socketCrash.connected === false) {
        this.notificationShow({
          type: "error",
          message: "Socket connection is not established. Please wait...",
        });
        return;
      }
      ...
      if (this.crashGame === null || this.crashGame.state !== "rolling") {
        this.notificationShow({
          type: "error",
          message: "Game is not in rolling state. Cannot cash out.",
        });
        return;
      }
      ...
      if (!userBet) {
        this.notificationShow({
          type: "error",
          message: "No active bet found to cash out.",
        });
        return;
      }
