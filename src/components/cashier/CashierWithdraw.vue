          ...
          <div class="content-element">
            ...
            <transition name="fade" mode="out-in">
              ...
              <div v-else class="element-content" key="data">
                ...
                <input
                  v-model="withdrawFiatAmount"
                  v-on:input="handleFiatInput"
                  @keypress="restrictToNumbers"
                  type="text"
                />
              </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        availableNetworks: [
          { code: "ethereum", name: "ERC20" },
          { code: "tron", name: "TRC20" },
          { code: "bsc", name: "BEP20" },
        ],
      };
    },
    methods: {
      ...
      fetchCryptoData() {
        ...
        if (!this.socketCashier.connected) {
          console.log(
            "Withdraw - Socket not connected yet, will retry. Connected:",
            this.socketCashier.connected
          );
          // Retry after a short delay
          setTimeout(() => this.fetchCryptoData(), 500);
          return;
        ...
        if (
          ...
          Object.keys(this.cashierCryptoData.prices || {}).length === 0
        ) {
          if (
            this.cashierCryptoData &&
            this.cashierCryptoData.loading === false
          ) {
            console.log(
              "Withdraw - Fetching crypto data... Socket connected:",
              this.socketCashier.connected
            );
            const data = {};
            this.cashierGetCryptoDataSocket(data);
            ...
            this.cashierCryptoData &&
            this.cashierCryptoData.loading === true
          ) {
            console.log("Withdraw - Crypto data is already loading...");
          } else {
            console.log(
              "Withdraw - Cannot fetch: cashierCryptoData:",
              this.cashierCryptoData
            );
          }
        } else {
      ...
      getCurrencyIcon(iconName) {
        try {
          ...
          if (cryptoImageMap[iconName]) {
            return require("@/assets/img/payments/crypto/" +
              cryptoImageMap[iconName] +
              ".png");
          }
      ...
      handleFiatInput(event) {
        ...
        if (!priceData) {
          // Try lowercase
          priceData =
            this.cashierCryptoData.prices[this.selectedCurrency.toLowerCase()];
        }
        if (!priceData) {
          // Try uppercase
          priceData =
            this.cashierCryptoData.prices[this.selectedCurrency.toUpperCase()];
        }
        if (!priceData) {
          this.withdrawAmount = "";
          return;
        ...
        if (
          priceData &&
          priceData.rate !== undefined &&
          priceData.rate !== null &&
          !isNaN(priceData.rate) &&
          priceData.rate > 0 &&
          fiatAmount > 0
        ) {
      ...
      handleCryptoInput(event) {
        ...
        // USDC has 1:1.001001 conversion rate with USD (1 USDC = 1.001001 USD)
        if (this.selectedCurrency === "usdc") {
          this.withdrawFiatAmount = parseFloat(cryptoAmount * 1.001001).toFixed(
            8
          );
          return;
        }
      ...
      handleConfirm() {
        ...
        const amount = parseFloat(this.withdrawAmount).toFixed(5);
        const data = {
    ...
    computed: {
      ...mapGetters(["generalSettings", "cashierCryptoData", "socketCashier"]),
      canConfirm() {
        return (
          this.withdrawAddress &&
          this.withdrawAddress.trim() !== "" &&
          this.withdrawAmount &&
      ...
      "cashierCryptoData.prices": {
        handler(newPrices, oldPrices) {
          ...
          if (newPrices && Object.keys(newPrices || {}).length > 0) {
            console.log(
              "Withdraw - Prices loaded! Available keys:",
