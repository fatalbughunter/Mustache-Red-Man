<template>
    <div class="modal-admin-crypto">
      ...
      <div class="crypto-element element-currency">
        ...
        <div class="element-content">
          <div class="content-inner">
            {{
              parseFloat(modalsData.transaction.amount) > 0
                ? parseFloat(modalsData.transaction.amount).toFixed(2)
                : parseFloat(modalsData.transaction.amount).toFixed(7)
            }}
          </div>
          <img
            v-bind:src="
              require('@/assets/img/cashier/' +
                modalsData.transaction.data.currency +
                '.png')
            "
            alt="icon"
          />
          <button
            v-on:click="modalCopyButton(modalsData.transaction.amount)"
            class="button-copy"
          >
            <svg
              width="16"
      ...
      <div class="crypto-element element-network">
        <div class="element-title">NETWORK</div>
        <div class="element-content">
          <div class="content-inner network-content">
            {{ modalsData.transaction.data.network }}
          </div>
        </div>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      modalApproveButton() {
        if (
          this.modalTransactionHash === null ||
          this.modalTransactionHash.trim() === ""
        ) {
          this.notificationShow({
            type: "error",
  ...
  .modal-admin-crypto .crypto-element.element-network .network-content {
